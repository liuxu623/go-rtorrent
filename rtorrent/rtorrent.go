...
func (r *RTorrent) GetStatus(t Torrent) (Status, error) {
	...
	// Finished
	results, err = r.xmlrpcClient.Call("d.timestamp.finished", t.Hash)
	if err != nil {
		return s, errors.Wrap(err, "d.timestamp.finished XMLRPC call failed")
	}
	s.Finished = time.Unix(int64(results.([]interface{})[0].(int)), 0)
	return s, nil
}
